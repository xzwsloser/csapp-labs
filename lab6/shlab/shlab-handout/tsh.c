...
void eval(char *cmdline);     // FINISHED
...
void eval(char *cmdline)
{
    ...
    if(build_in) return ;
    // 2. 开始执行命令
    sigset_t mask_all , mask_one , pre_mask;
    sigfillset(&mask_all);
...
int builtin_cmd(char **argv) 
{
    ...
    if(strcmp(cmd_name , QUIT) == 0) {
        ...
        listjobs(jobs);
        return 1;
    } else if(strcmp(cmd_name , BGCMD) == 0 || strcmp(cmd_name , FGCMD) == 0) {
        do_bgfg(argv);
        return 1;
    }
    return 0;
...
{
    // 获取到 jid
    if(argv[1] == NULL && (strcmp(argv[0] , BGCMD)) == 0) {
        printf("bg command requires PID or %%jobid argument\n");
        return ;
    } 

    if(argv[1] == NULL && (strcmp(argv[0] , FGCMD)) == 0) {
    ...
    if(argv[1][0] == '%') {
        ...
        sigprocmask(SIG_BLOCK , &mask_all , &pre_mask);  // 操作共享数据
...
void sigchld_handler(int sig) 
{
    // 函数需要检测到异常退出的状态,并且对于 jobs 中的数据进行删除操作
    sigset_t mask_all , pre_mask;
    sigfillset(&mask_all);
    int pid,status;
